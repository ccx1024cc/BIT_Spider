...
/**   
 * @Title: GRDNoticeContentExtractor.java 
 * @Package com.bit.ss.extractor 
 * @Description:  研究生院通知内容提取器
 * @author CCX
 * @date 2015年11月8日 上午11:23:15 
 * @version V1.0   
 */
public class GRDNoticeContentExtractor extends ContentExtractor {

	public GRDNoticeContentExtractor(String url, CloseableHttpClient client) {
		super(url, client);
	}
	...
	public Object extract() throws Exception {
		...
		try {
			...
			Element content = html.select("div#work").first();
				...
				if (fragment instanceof Element) {
					...
					if (!temp.select("img").isEmpty()) {
						...
						for (Element img : imgs) {
							// 背景图片不作处理
							if (!"bing-msbbp-copy-img".equals(img.attr("id"))
									&& !"bing-msbbp-search-img".equals(img.attr("id")))
			...
			// 提取附件
			Elements attachments = html.select("div.related a");
